...
for i in range(len(t)):
    ...
    u_wm += (dt/1.25) * (-u_wm + conv_wm + 8 * (f_pred * u_dest_pred) + h_u_wm)
    ...
    if plot_fields and (i % plot_every == 0 or i == len(t) - 1):
        ...
        lines[1].set_ydata(u_wm)

        axs[0, 0].set_title(f"Prediction - Time = {t[i]:.1f}, Trial {trial_number}")
        axs[0, 1].set_title(f"Working Memory - Time = {t[i]:.1f}")
...
field_histories = [
    (u_pred_history, 'System Prediction', (0, 2)),
    (u_wm_history, 'Working Memory', (-2, 5)),
]
